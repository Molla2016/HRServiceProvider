...
public class RegistrationController {
    ...
    /**protected boolean _resetPassword(String empID,String pass)throws NoSuchAlgorithmException{
        byte[] salt = getSalt();
        //System.out.println(pass);
        String query = "UPDATE Registration SET password =? WHERE EmployeeID = ?";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection con = DriverManager.getConnection(constring, username, password);
            PreparedStatement ps = con.prepareStatement(query);
            ps.setString(1, getEncrptedPassword(pass, salt));
